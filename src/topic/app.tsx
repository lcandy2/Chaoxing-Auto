...
export default function App() {
  ...
  const {isInActionFrame, actionSrc, setCurrentStatus, setCurrentPage} = useStatusStore();
  ...
  return (
      open && (<>
            <DraggableFloatingDialog title="学习通自动化" actions={<PanelActions/>} canBeClosed={!isInActionFrame}
                                     handleClose={handleClose}
            >
              <TabPanel/>
            </DraggableFloatingDialog>
            {(actionSrc) && (
                <DraggableFloatingDialog title="--执行讨论回复中--" canBeClosed={false}
